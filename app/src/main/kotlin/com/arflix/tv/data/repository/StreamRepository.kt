...
import com.arflix.tv.data.telegram.FenixSourceResolver
...
class StreamRepository @Inject constructor(
    ...
    private val fenixSourceResolver: FenixSourceResolver
) {
    ...
    private val ADDON_EPISODE_TIMEOUT_MS = 25_000L
    private val ADDON_SINGLE_STREAM_REQUEST_TIMEOUT_MS = 12_000L
    private val ADDON_NATIVE_ANIME_EPISODE_TIMEOUT_MS = 40_000L
    private val ADDON_NATIVE_ANIME_SINGLE_STREAM_REQUEST_TIMEOUT_MS = 20_000L
    private val ANIME_ID_LOOKUP_TIMEOUT_MS = 10_000L
    private val NATIVE_ANIME_ID_LOOKUP_TIMEOUT_MS = 15_000L
    // Longer timeouts for providers/requests known to be slower on large IPTV/legacy catalogs.
    private val ADDON_EXTENDED_TIMEOUT_MS = 60_000L
    private val ADDON_EXTENDED_EPISODE_TIMEOUT_MS = 80_000L
    private val ADDON_EXTENDED_SINGLE_STREAM_REQUEST_TIMEOUT_MS = 60_000L
    ...
    private val ADDON_AGGREGATOR_TIMEOUT_MS = 45_000L
    private val ADDON_AGGREGATOR_EPISODE_TIMEOUT_MS = 50_000L
    private val ADDON_AGGREGATOR_SINGLE_STREAM_REQUEST_TIMEOUT_MS = 30_000L
    ...
    fun resolveMovieStreamsProgressive(
        ...
        repositoryScope.launch {
            ...
            // Fenix Player: No cache for movies to ensure fresh Bot sources
            ...
            val prioritizedAddons = emptyList<Addon>() // Disable Stremio addons
            val telegramEnabled = false // Disable global search
            val fenixEnabled = true
            
            if (prioritizedAddons.isEmpty() && !telegramEnabled && !fenixEnabled) {
                Log.w(
                    TAG,
            ...
            var completed = 0
            val totalAddons = 1 // Only Bot search remains
            ...
            if (fenixEnabled) {
                launch {
                    val finalTmdbId = tmdbId ?: if (imdbId.startsWith("tmdb:")) imdbId.substringAfter("tmdb:").toIntOrNull() else null
                    
                    val fenixStreams = if (finalTmdbId != null) {
                        try {
                            fenixSourceResolver.resolveMovie(finalTmdbId)
                        } catch (e: Exception) {
                            if (e is kotlinx.coroutines.CancellationException) throw e
                            Log.e(TAG, "[StreamFetch][Movie] fenix resolve failed tmdbId=$finalTmdbId", e)
                            emptyList()
                        }
                    } else {
                        Log.w(TAG, "[StreamFetch][Movie] could not resolve TMDB ID for fenix from: $imdbId")
                        emptyList()
                    }

                    mutex.withLock {
                        aggregatedStreams.addAll(fenixStreams)
                        completed += 1
                        sendProgress()
                    }
                }
    ...
    fun resolveEpisodeStreamsProgressive(
        ...
        repositoryScope.launch {
            ...
            // Fenix Player: No cache for movies to ensure fresh Bot sources
            // Fenix Player: No cache for episodes to ensure fresh Bot sources
            if (false && !forceRefresh) {
            ...
            if (fenixEnabled) {
                launch {
                    ...
                    val fenixStreams = try {
                        if (finalTmdbId != null) {
                            fenixSourceResolver.resolveEpisode(finalTmdbId, season, episode)
                        } else {
                            // Episodes MUST have a tmdbId for Fenix. 
                            emptyList()
                        }
                    } catch (e: Exception) {
                        if (e is kotlinx.coroutines.CancellationException) throw e
                        Log.e(TAG, "[StreamFetch][Episode] fenix resolve failed tmdbId=$finalTmdbId", e)
                        emptyList()
                    }
    ...
    private suspend fun warmHttpConnection(stream: StreamSource) {
        ...
        // Skip pre-warming for local proxy to avoid redundant metadata prefetches
        if (rawUrl.contains("localhost") || rawUrl.contains("127.0.0.1")) return
    ...
    suspend fun isHttpStreamReachable(stream: StreamSource, timeoutMs: Long = 2_500L): Boolean =
        withContext(Dispatchers.IO) {
            ...
            // Skip reachability check for internal Telegram proxy to avoid 
            // triggering heavy prefetch/download tasks during background probes.
            ...
            val resolvedUrl = resolved.url?.trim().orEmpty()
            if (resolvedUrl.isBlank()) return@withContext false
    ...
    fun getAddonHealthBias(addonId: String): Int {
        ...
        val latencyBonus = when {
            stats.avgFetchLatencyMs in 1..4_000L -> 20
            stats.avgFetchLatencyMs in 4_001L..8_000L -> 8
            stats.avgFetchLatencyMs > 15_000L -> -20
            else -> 0
        }
